...
interface WhatsappMenuProps {
  settings: Pick<Setting, 'unidades'>
}

export function WhatsappMenu({ settings }: WhatsappMenuProps) {
  ...
  return (
    <div className="fixed right-14 bottom-14 z-50 max-[600px]:right-5 max-[600px]:bottom-5">
      ...
      {isOpen && (
        <div className="absolute bottom-20 right-0 rounded-xl p-4 flex flex-col gap-3 min-w-74">
          {settings.unidades?.map((unidade, index) => (
            <a
              key={unidade.id}
              href={`https://wa.me/${unidade.numero.toString().replaceAll(' ', '')}`}
      ...
      <button
        ...
        className="w-16 h-16 rounded-full bg-[#25D366] flex items-center justify-center shadow-lg hover:shadow-xl hover:scale-105 transition-all cursor-pointer"
